...
controller.listar = async (req, res) => {
    try {
        let dados = await Operador.find() // Traz todos os cursos cadastrados
        res.send(dados) // Vai com status HTTP 200
    }
    catch(erro) {
        console.log(erro)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        //Busca e substituição de conteúdo do objeto
        let ret = await Operador.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try{
        ...
        // Busca pelo id e exclusão
        let ret = await Operador.findByIdAndDelete(id)
